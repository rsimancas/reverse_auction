...
namespace NuvemWA.Models
{
    ...
    {
        public IList<Category> GetList(FieldFilters fieldFilters, string query, Sort sort, int page, int start, int limit, ref int totalRecords)
        {
            limit = limit + start;
            ...
            if (totalRecords > 0)
            {
                ...
                IList<Category> data = dt.ToList<Category>();
        ...
        public Category Add(Category data)
        {
            ...
            string sql = "INSERT INTO Categories ({0}) VALUES ({1}) " +
                "SELECT SCOPE_IDENTITY()";

            EnumExtension.setListValues(data, "CategoryKey", ref sql);
            ...
            Category returnData = Get(keyGenerated, oConn);
        ...
        public Category Update(Category data)
        {
            ...
            string sql = "UPDATE Categories SET {0} WHERE CategoryKey = @key";

            EnumExtension.setUpdateValues(data, "CategoryKey", ref sql);

            SqlCommand cmd = new SqlCommand(sql, oConn);

            cmd.Parameters.Add("@key", SqlDbType.Int).Value = data.CategoryKey;
            ...
            Category returnData = Get(data.CategoryKey, oConn);
        ...
        private Category Get(int id, SqlConnection oConn)
        {
            string sql = "SELECT * FROM Categories WHERE (CategoryKey = @id)";
            ...
            {
                IList<Category> data = dt.ToList<Category>();
                return data.FirstOrDefault<Category>();
            }

            return null;
        }

        public bool Remove(Category data)
        {
            SqlConnection oConn = null;
        ...
        private bool Remove(Category data, SqlConnection oConn)
        {
            string sql = "DELETE FROM Categories WHERE (CategoryKey = @key)";
